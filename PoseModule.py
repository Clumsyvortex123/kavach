...
class PoseDetector:
    ...
    def findPosition(self, img, draw=True, bboxWithHands=False):
        ...
        if self.results.pose_landmarks:
            ...
            if draw:
                ...
                cv2.circle(img, (cx, cy), 0, (255, 0, 0), cv2.FILLED)
    ...
    def findAngle(self, img, p1, p2, p3, draw=True):
        ...
        if draw:
            ...
            cv2.circle(img, (x1, y1), 1, (0, 0, 255), cv2.FILLED)
            cv2.circle(img, (x1, y1), 1, (0, 0, 255), 1)
            cv2.circle(img, (x2, y2), 1, (0, 0, 255), cv2.FILLED)
            cv2.circle(img, (x2, y2), 1, (0, 0, 255), 1)
            cv2.circle(img, (x3, y3), 1, (0, 0, 255), cv2.FILLED)
            cv2.circle(img, (x3, y3), 1, (0, 0, 255), 1)
            cv2.putText(img, str(int(angle)), (x2 - 50, y2 + 50),
                        cv2.FONT_HERSHEY_PLAIN, 2, (0, 0, 255), 2)
        return angle

    def findDistance(self, p1, p2, img, draw=True, r=1, t=3):
        x1, y1 = self.lmList[p1][1:]
        x2, y2 = self.lmList[p2][1:]
...
def main():
    ...
    while True:
        ...
        if bboxInfo:
            ...
            cv2.circle(img, center, 0, (255, 0, 255), cv2.FILLED)
